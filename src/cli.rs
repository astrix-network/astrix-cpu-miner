...
pub struct Opt {
    ...
    #[clap(short = 's', long = "astrixd-address", default_value = "127.0.0.1", display_order = 1)]
    /// The IP of the astrixd instance
    pub astrixd_address: String,

    #[clap(long = "devfund", display_order = 6)]
    /// Mine a percentage of the blocks to the Astrix devfund [default: Off]
    ...
    /// Astrixd port [default: Mainnet = 34150, Testnet = 16210]
    port: Option<u16>,
    ...
    /// Output logs in alternative format (same as astrixd)
    pub altlogs: bool,
}
...
impl Opt {
    pub fn process(&mut self) -> Result<(), Error> {
        if self.astrixd_address.is_empty() {
            self.astrixd_address = "127.0.0.1".to_string();
        }

        if !self.astrixd_address.starts_with("grpc://") {
            IpAddr::from_str(&self.astrixd_address)?;
            let port = self.port();
            self.astrixd_address = format!("grpc://{}:{}", self.astrixd_address, port);
        }
        log::info!("Astrixd address: {}", self.astrixd_address);

        Ok(())
    }

    fn port(&mut self) -> u16 {
        *self.port.get_or_insert(if self.testnet { 16210 } else { 34150 })
    }
