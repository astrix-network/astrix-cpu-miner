use crate::{
    miner::MinerManager,
    proto::{
        astrixd_message::Payload, rpc_client::RpcClient, GetBlockTemplateRequestMessage, GetInfoRequestMessage,
        AstrixdMessage,
    },
...
#[allow(dead_code)]
pub struct AstrixdHandler {
    client: RpcClient<TonicChannel>,
    pub send_channel: Sender<AstrixdMessage>,
    stream: Streaming<AstrixdMessage>,
...
impl AstrixdHandler {
    ...
    pub async fn client_send(&self, msg: impl Into<AstrixdMessage>) -> Result<(), SendError<AstrixdMessage>> {
        self.send_channel.send(msg.into()).await
    }

    pub async fn client_get_block_template(&mut self) -> Result<(), SendError<AstrixdMessage>> {
    ...
    pub async fn listen(&mut self, miner: &mut MinerManager, shutdown: ShutdownHandler) -> Result<(), Error> {
        while let Some(msg) = self.stream.message().await? {
            ...
            match msg.payload {
                ...
                None => warn!("astrixd message payload is empty"),
            }
        }
        Ok(())
    }
    ...
    async fn handle_message(&mut self, msg: Payload, miner: &mut MinerManager) -> Result<(), Error> {
        match msg {
            ...
            Payload::GetInfoResponse(info) => info!("Astrixd version: {}", info.server_version),
