...
use crate::{
    cli::Opt, client::AstrixdHandler, miner::MinerManager, proto::NotifyNewBlockTemplateRequestMessage, target::Uint256,
};

mod cli;
mod client;
mod astrixd_messages;
mod miner;
mod pow;
mod swap_rust;
mod target;
...
async fn main() -> Result<(), Error> {
    ...
    while !shutdown.is_shutdown() {
        let mut client =
            AstrixdHandler::connect(opt.astrixd_address.clone(), opt.mining_address.clone(), opt.mine_when_not_synced)
                .await?;
        ...
        warn!("Disconnected from astrixd, retrying");
    }
    Ok(())
}
